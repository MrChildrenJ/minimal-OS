...
// ============================================================
// 5. Exception Handler
// (registered in kernel_main via WRITE_CSR(stvec, kernel_entry))
// Flow: kernel_entry (ASM) → handle_trap → handle_syscall
// ============================================================
...
// Validate that [addr, addr+len) lies entirely within user-accessible pages.
// Walks the two-level page table and checks the U bit on each page.
static bool is_user_addr(uint32_t *table1, uint32_t addr, size_t len) {
    uint32_t start = addr & ~(PAGE_SIZE - 1);
    for (uint32_t va = start; va < addr + len; va += PAGE_SIZE) {
        uint32_t vpn1 = (va >> 22) & 0x3ff;
        if ((table1[vpn1] & PAGE_V) == 0)
            return false;
        uint32_t *table0 = (uint32_t *) ((table1[vpn1] >> 10) * PAGE_SIZE);
        uint32_t vpn0 = (va >> 12) & 0x3ff;
        if ((table0[vpn0] & (PAGE_V | PAGE_U)) != (PAGE_V | PAGE_U))
            return false;
    }
    return true;
}
...
void handle_syscall(struct trap_frame *f) {
    ...
    switch (sysno) {
        ...
        case SYS_WRITEFILE: {
            ...
            int len = f->a2;
            if (len <= 0 || len > 1024 ||
                !is_user_addr(current_proc->page_table, (uint32_t) filename, 100) ||
                !is_user_addr(current_proc->page_table, (uint32_t) buf, (size_t) len)) {
                f->a0 = -1;
                break;
            }
...
// ============================================================
// 6. Disk I/O
// (virtio_blk_init called in kernel_main)
// ============================================================
...
// ============================================================
// 7. File System
// (fs_init called in kernel_main after virtio_blk_init)
// ============================================================
...
// ============================================================
// 8. Page Table
// (map_page used in create_process to map kernel + user memory)
// ============================================================
...
// ============================================================
// 9. User Mode
// (user_entry is pushed as ra onto every new process's kernel stack;
//  switch_context pops it and jumps here, which then sret's into U-mode)
// ============================================================
...
// ============================================================
// 10. Context Switch
// (called by yield to swap sp and execution between two processes)
// ============================================================
...
// (create_process called in kernel_main to set up idle + shell)
// ============================================================
...
// ============================================================
// 13. Context Switch Benchmark
// (run_ctx_switch_bench called in kernel_main before launching shell)
